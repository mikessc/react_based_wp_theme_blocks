...
import { InspectorControls, MediaUploadCheck, MediaUpload, InnerBlocks, useBlockProps } from '@wordpress/block-editor';
...
const DisplayMedia = ({media, className}) => { 
	if (media) { 
		...
		if (media.type == 'video') { 
			return (
				<video className={className} width='400' height='300' autoPlay loop muted >
	...
	return (
		<MediaUploadCheck>
			<MediaUpload
				...
				render={({open}) => {
					return <div className='wp-block-tbwa-blocks-section-with-media-media-select' >
						<div onClick={open}>
		...
		<>
			<InspectorControls >
				<PanelBody 
					...
					initialOpen={false} 
					className="wp-block-tbwa-blocks-section-with-media-inspector" >
						<SelectControl
